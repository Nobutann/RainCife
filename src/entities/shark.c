...
static void FireProjectile(Shark *shark, Rectangle playerRect) {
    for (int i = 0; i < MAX_WATER_BALLS; i++) {
        if (!shark->balls[i].active) {
            ...
            shark->balls[i].speed = 20.0f * 60.0f; // Speed for shooting attack
...
static void FireBubble(Shark *shark) {
    for (int i = 0; i < MAX_WATER_BALLS; i++) {
        if (!shark->balls[i].active) {
            ...
            shark->balls[i].direction = (Vector2){ 0, 1.0f }; // Straight down
            shark->balls[i].speed = 9.5f * 60.0f; // Original speed
            shark->balls[i].active = true;
            break;
        }
    }
...
void UpdateShark(Shark *shark, Rectangle playerRect, float deltaTime, int screenWidth, int screenHeight) {
    ...
    switch (shark->state) {
        case SHARK_IDLE:
            ...
            if (shark->timer > 2.0f) {
                ...
                if (chance <= 33) {
                    ...
                    // Angle ~60 degrees upwards to the left. 
                    // tan(60) = 1.732.
                    shark->velocity.x = -850.0f;
                    shark->velocity.y = -1472.0f; // 850 * 1.732
                    shark->arcDrops = 0;
                }
                shark->timer = 0.0f;
            }
        ...
        case SHARK_PREP_LEFT:
            shark->rect.x -= 1500.0f * deltaTime; // Increased from 1000.0f
        ...
        case SHARK_DASH_RIGHT:
            shark->rect.x += 1500.0f * deltaTime; // Matched to ida (SHARK_PREP_LEFT) and standard return
        ...
        case SHARK_DASH_LEFT:
            ...
            float returnSpeed = 1200.0f; // Reverted to standard value
        ...
        case SHARK_ARC_ATTACK:
            ...
            shark->velocity.y += 1300.0f * deltaTime; // Gravity
            shark->timer += deltaTime;
            ...
            if (shark->rect.y > (float)screenHeight + 100.0f) {
                // Landed on the left side, now dash back to the right
                shark->rect.y = shark->startPos.y;
                shark->state = SHARK_DASH_RIGHT;
                shark->timer = 0.0f;
            }
