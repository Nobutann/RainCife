...
void UpdateShark(Shark *shark, Rectangle playerRect, float deltaTime, int screenWidth, int screenHeight) {
    ...
    float ballSpeed = 9.5f * 60.0f;
    for (int i = 0; i < MAX_WATER_BALLS; i++) {
        if (shark->balls[i].active) {
            shark->balls[i].rect.x += shark->balls[i].direction.x * ballSpeed * deltaTime;
            shark->balls[i].rect.y += shark->balls[i].direction.y * ballSpeed * deltaTime;
    ...
    switch (shark->state) {
        case SHARK_IDLE:
            ...
            if (shark->timer > 2.0f) {
                ...
                if (chance <= 33) {
                    ...
                    shark->velocity.x = -750.0f;
                    shark->velocity.y = -1500.0f;
                    shark->arcDrops = 0;
                }
                shark->timer = 0.0f;
            }
        ...
        case SHARK_PREP_LEFT:
            shark->rect.x -= 1000.0f * deltaTime;
        ...
        case SHARK_ARC_ATTACK:
            ...
            float startX = shark->startPos.x;
            float endX = -shark->rect.width;
            float totalDist = startX - endX;

            if (shark->arcDrops == 0 && shark->rect.x < startX - totalDist * 0.25f) {
                FireProjectile(shark, playerRect);
                shark->arcDrops++;
            } else if (shark->arcDrops == 1 && shark->rect.x < startX - totalDist * 0.50f) {
                FireProjectile(shark, playerRect);
                shark->arcDrops++;
            } else if (shark->arcDrops == 2 && shark->rect.x < startX - totalDist * 0.75f) {
                FireProjectile(shark, playerRect);
                shark->arcDrops++;
            }

            if (shark->rect.y > (float)screenHeight + 100.0f) {
                shark->state = SHARK_WAIT_RETURN;
                shark->timer = 0.0f;
            }
            break;
