...
void InitHairyLeg(HairyLeg *leg, Vector2 startPosition, float groundY,
                  float scale) {
  ...
  leg->rect = (Rectangle){startPosition.x, groundY - emptyBottom - hitboxH,
                          40.0f * scale, hitboxH};
  leg->state = HL_IDLE;
  leg->timer = 0.0f;
  leg->health = 100;
...
void UpdateHairyLeg(HairyLeg *leg, Rectangle playerRect, float deltaTime,
  ...
  if (leg->state != HL_JUMPING_UP && leg->state != HL_FALLING &&
      leg->state != HL_HANGING && leg->state != HL_SWEEPING) {
  ...
  case HL_IDLE:
    ...
    if (leg->timer > 1.0f) {
      ...
      bool encurraladoEsquerda =
          (playerRect.x < 300 && leg->direction == -1 && leg->rect.x < 500);
      bool encurraladoDireita =
          (playerRect.x > screenW - 300 && leg->direction == 1 &&
           leg->rect.x > screenW - 500);
  ...
  case HL_FALLING:
    ...
    if (leg->rect.y >= leg->groundY - emptyBottom - defaultHitboxH) {
      ...
      leg->waveLeft = (Shockwave){
          {leg->rect.x, leg->rect.y + currentSpriteH * 0.75f, waveW, waveH},
          {600, 0},
          true};
      leg->waveRight =
          (Shockwave){{leg->rect.x + leg->rect.width,
                       leg->rect.y + currentSpriteH * 0.75f, waveW, waveH},
                      {600, 0},
                      true};
  ...
  case HL_SWEEPING:
    ...
    float footX = (leg->direction == 1) ? (leg->rect.x + leg->rect.width)
                                        : (leg->rect.x - sweepFootWidth);
    leg->kickHitbox =
        (Rectangle){footX, leg->rect.y, sweepFootWidth, leg->rect.height};
    leg->isKickActive = false;
  ...
  case HL_KICKING:
    ...
    if (leg->timer < 0.4f) {
      ...
      float hitboxX = (leg->direction == 1) ? (leg->rect.x + leg->rect.width)
                                            : (leg->rect.x - larguraHitbox);
      leg->kickHitbox =
          (Rectangle){hitboxX, leg->rect.y + currentSpriteH * 0.7f,
                      larguraHitbox, alturaHitbox};
      leg->isKickActive = true;
    } else if (leg->timer < 1.2f) {
      leg->isKickActive = false;
  ...
  if (leg->state != HL_SWEEPING) {
    ...
    if (isAttack &&
        leg->currentAnim->currentFrame >= leg->currentAnim->frameCount) {
      leg->currentAnim->currentFrame = leg->currentAnim->frameCount - 1;
    }
  }
}
...
void DrawHairyLeg(HairyLeg *leg, float scale) {
  ...
  if (leg->waveLeft.active) {
    DrawRectangleLinesEx(leg->waveLeft.rect, 2.0f,
                         BLUE); // Onda de choque esquerda
  }
  if (leg->waveRight.active) {
    DrawRectangleLinesEx(leg->waveRight.rect, 2.0f,
                         BLUE); // Onda de choque direita
  }
}

void UnloadHairyLeg(HairyLeg *leg) { UnloadHairyLegSprites(&leg->sprites); }
